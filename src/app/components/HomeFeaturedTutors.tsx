...
import { featuredTutors, Tutor } from '@/lib/data/tutors';
...
// readable label. The card used to reference a non-existent `tutor.mode`
// field, which is why the badge showed up blank.
function getModeLabel(teachingMode: Tutor['teachingMode']): string {
  const hasHome = teachingMode.includes('home') || teachingMode.includes('both');
  const hasOnline = teachingMode.includes('online') || teachingMode.includes('both');

  if (hasHome && hasOnline) return 'Home & Online';
  if (hasHome) return 'Home';
  if (hasOnline) return 'Online';
  return '';
}

// Joins an array field into a short, readable line, truncating with a
// "+N" suffix so a tutor with many subjects/classes doesn't overflow the
// card. Previously these arrays were rendered directly as JSX children,
// which React just concatenates with no separator at all.
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {featuredTutors?.map((tutor) => {
            const modeLabel = getModeLabel(tutor.teachingMode);
